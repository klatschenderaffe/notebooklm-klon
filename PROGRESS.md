...
## 2026-09-21 — Live-Vorfall: "Antwort nicht in den Quellen enthalten", obwohl sie es war — zwei echte RAG-Bugs

Nachdem der Chat dank Timeout/Fallback wieder zuverlässig antwortete, meldete der Nutzer ein neues, inhaltliches Problem: eine Frage zu einer hochgeladenen `.md`-Quelle wurde mit "nicht in den Quellen enthalten" beantwortet, obwohl die Antwort nachweislich im Dokument stand. Statt zu raten, wurde die komplette RAG-Pipeline (Chunking + Embedding + Similarity-Filter) lokal 1:1 mit der echten Datei nachgebaut (`chunk_text()` + `embed_texts()` aus dem echten Code, keine Mocks) — das deckte zwei unabhängige, reale Bugs auf.

**Bug 1 — `chunk_text()` hängt sich bei Listen ohne Leerzeilen fast auf:** Aus der betroffenen Datei entstanden **23 Chunks** statt der erwarteten ~7-8. Die Chunks 6–21 waren praktisch identisch (150, 149, 148 … 135 Zeichen, jeweils nur 1 Zeichen kürzer). **Root Cause:** Ein Markdown-Abschnitt aus Listeneinträgen ohne Leerzeilen dazwischen (eine Quellen-/Linkliste) ließ `text.rfind(" ", start, end)` eine Wort-Grenze knapp hinter `start` finden; die Bedingung `boundary > start` akzeptierte das, wodurch der Chunk auf wenige Zeichen schrumpfte und `start` pro Schleifendurchlauf nur um 1 Zeichen vorrückte. **Fix:** Eine gefundene Grenze wird nur noch verwendet, wenn sie mindestens die Hälfte von `chunk_size` Fortschritt bringt — sonst wird stattdessen hart bei `chunk_size` abgeschnitten. Regressionstest mit einer künstlichen Linkliste ohne Leerzeilen ergänzt.

**Bug 2 — die eigentliche Ursache des gemeldeten Symptoms:** Selbst nach Bug-1-Fix lag die Ähnlichkeit des inhaltlich besten, tatsächlich relevanten Chunks für eine natürlich formulierte Frage nur bei **0,62** — der bisherige globale Schwellenwert lag bei **0,7**. Die Antwort war also nachweislich eingebettet, wurde aber vom Filter aussortiert, bevor sie überhaupt bei der KI ankam. Der 0,7-Wert war ursprünglich an einem einzigen günstigen Testfall kalibriert (siehe Eintrag weiter oben) und generalisiert nicht auf reale Fragen gegen Markdown-lastige Quellen. **Wichtiger Befund bei der Recherche:** Der bereits bestehende Regressionstest für einen anderen Live-Fund (`test_chat_filters_out_low_similarity_matches`, thematisch verwandter aber irrelevanter Treffer bei similarity=0,53) zeigt, dass sich "relevant" (0,62 in diesem neuen Fall) und "irrelevant" (0,53 im alten Fall) im Ähnlichkeits-Score sogar überschneiden — ein einzelner globaler Schwellenwert kann beide Fälle strukturell nicht zuverlässig trennen.

**Fix (robuster statt nur ein neuer Schwellenwert):** `match_chunks` liefert bereits absteigend nach Ähnlichkeit sortiert (`order by chunks.embedding <=> query_embedding`). Der beste verfügbare Treffer wird jetzt in `chat.py` immer verwendet, unabhängig davon, ob er den Schwellenwert erreicht — der Schwellenwert filtert nur noch zusätzliche, schwächere Treffer. Damit bleibt der bereits bestehende Schutz vor thematisch verwandten aber irrelevanten Zusatztreffern erhalten (der alte Regressionstest bleibt unverändert grün), während der tatsächlich beste Treffer nie mehr komplett verworfen wird, egal wie das Embedding im Einzelfall ausfällt. Zwei neue Regressionstests (bester Treffer unter Schwellenwert wird trotzdem verwendet; leere Trefferliste führt nicht zum Absturz).
