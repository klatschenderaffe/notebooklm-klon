...
def create_presentation(request: PresentationRequest) -> StreamingResponse:
    ...
    outline = generate_presentation_outline(request.topic, [c["content"] for c in chunks])
    pptx_bytes = build_presentation(outline.get("title", request.topic), outline.get("slides", []))
